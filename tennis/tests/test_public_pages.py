...
class PublicPagesTests(TestCase):
    def test_top_page_explains_the_service_and_links_to_public_pages(self):
        ...
        self.assertContains(response, "テニスサークルの運営を、")
        self.assertContains(response, "練習会スケジュール")
        self.assertContains(response, "出席メンバーの中から")
        self.assertContains(response, "個人＆クラブ")
        self.assertContains(response, "ログイン不要！")
        self.assertContains(response, "score-entry-illustration.")
        self.assertContains(response, "tennis/landing/attendance-illustration.")
        self.assertContains(response, "tennis/landing/matchup-illustration.")
